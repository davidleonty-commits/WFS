...
|---|---|---|---|---|
| `slack_schedule_message(text, channel, jitter_minutes, send_at_mt, thread_ts, mentions)` | 107 | Posted a Slack message (immediately, or at `send_at_mt`, or into a thread via `thread_ts`) through the WFS Group workspace bot. Returned `ok`, a queue id, resolved channel label, planned send time. | The claude.ai Slack connector: `slack_send_message` for immediate sends (pass `thread_ts` for threaded replies), `slack_schedule_message` when `send_at_mt` is set. Treat the returned `ts` as the delivery proof the prompts' "DELIVERY VERIFICATION" steps ask for. Mentions stay as inline `<@USERID>` tokens; there is no separate `mentions` field. `jitter_minutes` was always 0; drop it. | None beyond connecting Slack in claude.ai under the new director's account. The connector posts as them, which matches how the Lovable app posted as Cayden. |
...
- **One Slack sender only.** The prompts say "the Lovable WFS connector, never the native Slack connector" because two senders would race. After the Section 1A swap the rule becomes: the claude.ai Slack connector, never a second sender. Keep the single-sender discipline; change the name.
