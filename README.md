...
3. **Delete the "SLACK SENDER LOCK" wording** that says "Never the native Slack connector." That rule existed because the Lovable app held the workspace bot token. With direct API access, the native Slack API IS the sender.
...
|---|---|---|---|---|
| `slack_schedule_message(text, channel, jitter_minutes, send_at_mt, thread_ts, mentions)` | 107 | Posted a Slack message (immediately, or at `send_at_mt`, or into a thread via `thread_ts`) through the WFS Group workspace bot. Returned `ok`, a queue id, resolved channel label, planned send time. | Slack Web API: `chat.postMessage` for immediate sends (pass `thread_ts` for threaded replies), `chat.scheduleMessage` when `send_at_mt` is set. Use the Slack MCP connector in claude.ai or call the API directly. Treat `ok: true` plus a returned `ts` as the delivery proof the prompts' "DELIVERY VERIFICATION" steps ask for. Mentions stay as inline `<@USERID>` tokens; there is no separate `mentions` field. | Slack bot token for the WFS Group workspace with `chat:write`, `chat:write.public`, `channels:read`, `channels:history`, `im:write`, `users:read`. |
| `slack_list_pending`, `slack_cancel_message` | 25 | Listed or cancelled messages queued in the Lovable app. | `chat.scheduledMessages.list` and `chat.deleteScheduledMessage`. The prompts already say `slack_list_pending` is never a delivery test; with direct API you can drop these calls entirely. | Same Slack token. |
| `slack_read_channel`, `slack_read_thread`, `slack_send_message` | 14 | Read `#payments` and the reps channel; read a thread; one-off send. | `conversations.history`, `conversations.replies`, `chat.postMessage`. | Same Slack token. |
...
- **One Slack sender only.** The prompts say "the Lovable WFS connector, never the native Slack connector" because that app held the bot token. After the Section 1A swap the rule becomes: the Slack API with the bot token, never a personal user token, never a second sender. Keep the single-sender discipline; change the name.
