...
Default (scheduled or "send it"): send to the director's own Slack DM with `slack_send_message` on the claude.ai Slack connector (the claude.ai Slack connector, which posts as you), `channel` set to the director's Slack member ID (DIRECTOR_SLACK_ID U0BUZ6C0C91). Send the run summary as message one and the cards as message two. Split at a card boundary, never mid-card, only if a message would exceed about 5000 characters.
