...
5. **Escalation packets go to David only.** This loop never sends anything to leadership, the rep, or any channel on its own. It builds the packet, delivers it to the director's DM with `slack_send_message` on the Slack connector (test-mode routing rules apply), and he decides. The ledger is this skill's only write target.

## The ledger

A Google Sheet named **Compliance Ledger** in David's Drive. On first run, search Drive for it; if missing, create it and report the new file. Columns:
...
| escalated | blank, or date the packet went to David |
...
- **2 of the same type in 30 days:** formal flag. The rep's next written review carries a compliance section naming both instances with quotes, and the trend line marks the threshold hit. Packet gets built and sent to David's DM.
- **3 total clear entries in 30 days, or 2 of the same type after a coached date:** escalation packet to David's DM marked as leadership-ready, with the recommendation that this leaves the coaching lane. Repeat-after-coaching is the aggravator that matters most, because it converts "did not know" into "was told and continued."
...
Built automatically at any threshold, delivered to David's DM:
