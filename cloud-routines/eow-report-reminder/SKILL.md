...
STEP 2 - send the DM
Send exactly one Slack DM with `slack_send_message`:
- `channel` = DIRECTOR_SLACK_ID
- `text` = the message below
- no `thread_ts`, and nothing scheduled, so it goes out immediately

One sender only, the connector above. Never post to a channel. The only destination ever allowed is DIRECTOR_SLACK_ID.
...
Confirm the send from the `slack_send_message` RETURN VALUE and nothing else: `ok` = true, a non-empty `ts`, and a returned `channel` that resolves to DIRECTOR_SLACK_ID. Never confirm with a follow-up read.

If the send fails outright, retry `slack_send_message` ONCE after a short pause. There is no second sender, so if the retry also fails, send nothing else and report the failure in STEP 4.
