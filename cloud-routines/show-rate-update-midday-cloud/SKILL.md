...
created: 2026-07-16
connectors_required: Callix, Slack
---

SCHEDULED TASK: Show Rate Update, Mid-Day, Webinar + S2C
Purpose: mid-day show rate report for TODAY (Webinar + S2C closer calls) with time-based show rates from OnceHub and live-call counts from Callix, delivered via one Slack message. METRICS ONLY: no call recaps or summaries.
...
3. READ-ONLY on all sources (OnceHub, Callix, everything). Never create, edit, cancel, reschedule, delete, sort, filter, comment on, or otherwise modify any source object. NO writes of any kind other than the Slack sends in rule 1.
...
10. CALLIX ACCESS FALLBACK (a METHOD allowance under HARD RULE 7; NOT a policy change, NOT a last-resort trigger): if a native Callix MCP tool errors or is unreachable, access the SAME Callix data through the Callix REST API directly (read-only GETs), per the STEP 0 CALLIX ACCESS FALLBACK block. Callix remains the source of truth for live counts; the REST call is only a different door to it.
...
CALLIX ACCESS FALLBACK (method, not policy; allowed by HARD RULE 7): If ANY native Callix MCP tool (list_calls) errors or is unreachable (e.g. 403 mcp_request_blocked, 502, "MCP server not connected"), retry once, then access the SAME Callix data through the Callix REST API directly instead of treating Callix as down. Callix stays the source of truth for live counts; this is only a different access METHOD, and is NOT a SELF-HEAL last-resort condition. REST usage (header `Authorization: Bearer $CALLIX_API_KEY`, GET only):
- Meetings: `GET $CALLIX_API_BASE/calls` with query {from_date, to_date, page, page_size:100}. The API caps ~10 rows per page, so paginate by incrementing page until `next` is null. Qualify calls with the same rules as STEP 2 (state completed, duration > 900s, title contains a TikTok Wiz Consultation, exclude Introductions/sync/TikTok Shop/CANCELED). Note that duration is null for scheduled/not-yet-held calls and "not_recorded"/silent recordings have duration 0; both are excluded.
...
Only if BOTH the native Callix MCP AND the Callix REST API are down for the whole run is Callix a last-resort condition.

=====================================================
OBJECTIVE
=====================================================
TODAY ONLY. Scheduled counts: OnceHub via a fully paginated `GET /v2/bookings` sweep grouped by master page (`GET /v2/booking-calendars/{id}` resolves unlabeled ids); same numbers the TTW Daily Lead Flow Report is built from. Live counts: Callix (list_calls).
TIME-BASED show rate (the retired "divide total in half" heuristic is forbidden): per bucket, Scheduled (in total for today) = closer bookings with starting_time anywhere in today's ET day; Scheduled (so far today) = closer bookings with starting_time from today 00:00 ET up to and including NOW (ET), i.e. calls due to have started by pull time; Scheduled Calls Remaining = Total minus So-far. Show Rate = Callix live calls / So-far.
Determine "today" and "now" explicitly at runtime. OnceHub slices use the America/New_York (ET) calendar day and an ET "now" cutoff, matching the Lead Flow report day boundary (DST-safe: -04:00 EDT, -05:00 EST). The Callix live-call day window is Mountain Time per STEP 2.
...
=====================================================
STEP 2: Live calls + per-rep breakdown + qualifying set (Callix)
=====================================================
Callix list_calls (meeting_state=completed) across the full Mountain-Time day (UTC window: that day 06:00Z to next day 06:00Z), paginating all pages. If the native Callix tool is blocked, use the Callix REST API per the STEP 0 CALLIX ACCESS FALLBACK (same data, same qualifying rules). page_size caps at 10 and busy days span many pages: run retrieval and filtering in a subagent that returns only the qualifying list, keeping the main context clean.
...
Split: S2C Live Calls = qualifying calls whose title contains "S2C"; Webinar Live Calls = qualifying calls whose title does NOT contain "S2C". (Callix titles do not distinguish S2C from DM S2C, so both land in S2C Live, consistent with the combined S2C scheduled figure.)
