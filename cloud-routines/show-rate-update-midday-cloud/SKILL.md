...
TEST_TARGET: @cayden.johnson
  (Owner's DM; the only destination allowed in TEST. The bare handle "@cayden" does not resolve on the WFS server. On error or transient 503, retry once, then use Slack user ID U092C85GA4D, Cayden Johnson's DM, which resolves reliably.)
...
DELIVERY ENDPOINT
=====================================================
Never use the native Slack connector. All Slack delivery goes through the Lovable WFS Slack connector's slack_schedule_message (it owns channel/@-mention resolution, jitter, dispatch). Destination = TEST_TARGET if TEST, LIVE_TARGET if LIVE; never LIVE_TARGET while TEST. Call with text = the finished message, channel = that destination, jitter_minutes = JITTER_MINUTES, no send_at_mt. Capture queue id, resolved channel label, planned send time. Send exactly once.
ERROR HANDLING: 401/Unauthorized -> report that WFS_MCP_TOKEN needs refreshing in Settings -> AI Agent Access. "not in the approved allowlist" -> report; never improvise a different channel. "Slack is not connected" -> report; do not retry endlessly. slack_schedule_message missing entirely -> report the Lovable WFS MCP connector is not connected; never improvise another delivery path. Connector-down conditions are last-resort conditions for the SELF-HEAL loop, not license to use another sender.
...
=====================================================
1. Delivery via the Lovable WFS connector's slack_schedule_message ONLY; never the native Slack connector. The single send to the DELIVERY_MODE target (plus, last-resort only, one QA FAILURE DM to the owner, and the slack_list_pending verification read) are the ONLY Slack actions.
...
9. COLLECTIVE SHOW-RATE DM OVERRIDE: After STEP 3, compute Collective Show Rate = (Webinar Live Closer Calls + S2C Live Closer Calls) / (Webinar Scheduled so-far + S2C Scheduled so-far), percent to one decimal. If it is below 40%, the delivery destination is ALWAYS the owner's DM (TEST_TARGET, fallback U092C85GA4D), even when DELIVERY_MODE is LIVE; this OVERRIDES LIVE_TARGET routing. The report is NEVER posted to LIVE_TARGET when the collective rate is under 40%. If the combined so-far denominator is 0, the collective rate is N/A and routing follows DELIVERY_MODE normally. This does not change the single-send rule (still exactly one report send) and does not add the collective rate to the message body (F2 unchanged).
10. AVOMA ACCESS FALLBACK (a METHOD allowance under HARD RULE 7; NOT a policy change, NOT a last-resort trigger): if a native Avoma MCP tool errors or is unreachable, access the SAME Avoma data through the Lovable WFS connector's read-only Avoma REST proxy avoma_api, per the STEP 0 AVOMA ACCESS FALLBACK block. Avoma remains the source of truth for live counts; the proxy is only a different door to it.
...
Confirm oncehub_booking_counts, oncehub_get_master_page (WFS OnceHub MCP), list_meetings, get_current_datetime (Avoma MCP), and the Lovable WFS Slack connector are reachable; if one is down, that is a SELF-HEAL condition (and if down all run, a last-resort condition). Never fall back to a browser for any data.

AVOMA ACCESS FALLBACK (method, not policy; allowed by HARD RULE 7): If ANY native Avoma MCP tool (list_meetings, get_current_datetime) errors or is unreachable (e.g. 403 mcp_request_blocked, 502, "MCP server not connected"), retry once, then access the SAME Avoma data through the Lovable WFS connector's read-only Avoma REST proxy avoma_api instead of treating Avoma as down. Avoma stays the source of truth for live counts; this is only a different access METHOD, and is NOT a SELF-HEAL last-resort condition. Proxy usage:
- Meetings: avoma_api path "/v1/meetings/" with query {from_date, to_date, page, page_size:100}. The API caps ~10 rows per page, so paginate by incrementing page until `next` is null. Qualify calls with the same rules as STEP 2 (state completed, duration > 900s, title contains a TikTok Wiz Consultation, exclude Introductions/sync/TikTok Shop/CANCELED). Note that duration is null for scheduled/not-yet-held calls and "not_recorded"/silent recordings have duration 0; both are excluded.
...
Only if BOTH the native Avoma MCP AND the avoma_api proxy are down for the whole run is Avoma a last-resort condition.

=====================================================
OBJECTIVE
=====================================================
TODAY ONLY. Scheduled counts: OnceHub via oncehub_booking_counts grouped by master page (oncehub_get_master_page resolves unlabeled ids); same numbers the TTW Daily Lead Flow Report is built from. Live counts: Avoma (list_meetings).
...
Make TWO oncehub_booking_counts calls, identical except date_to, both with date_field = starting_time and status = ["scheduled","rescheduled","completed","no-show"]:
- CALL TOTAL: date_from = today 00:00:00 ET, date_to = today 23:59:59 ET.
- CALL SO-FAR: date_from = today 00:00:00 ET, date_to = NOW_ET.
oncehub_booking_counts excludes in_trash by default and is known-good for daily starting_time slices. Read counts per master page from rows in BOTH calls; resolve any row whose label is a raw master_page_id with oncehub_get_master_page. Unattributed bookings (total_all minus total_attributed) are EXCLUDED from every count; note the number (from the TOTAL call) as an anomaly. OnceHub is live, so back-testing a past day will not reproduce a morning snapshot; only ever read TODAY live.
...
5. LAST RESORT ONLY: if after 2 cycles the failure persists AND the cause is a genuine external outage or source problem no method can fix (required connector hard-down all run, OnceHub returning no data that will not reconcile, a total that will not reconcile), send ONE short QA FAILURE DM to the owner (TEST_TARGET, fallback U092C85GA4D) naming the failed checks, the values, and every fix attempted. QA FAILURE reports ALWAYS go to the owner's DM, even when DELIVERY_MODE is LIVE. Then stop without delivering.
...
=====================================================
Destination per DELIVERY_MODE (never LIVE_TARGET while TEST), and per HARD RULE 9 the destination is the owner's DM whenever the Collective Show Rate is below 40%, even in LIVE mode (overrides LIVE_TARGET). Make ONE slack_schedule_message call: channel = that destination, text = the STEP 5 body (clean body only), jitter_minutes = JITTER_MINUTES, agent = "claude-cowork", no send_at_mt. Send exactly once.
DELIVERY VERIFICATION (the run is NOT complete until this passes): (1) the call returned ok = true with a non-empty queue id and a resolved channel label; (2) the resolved label matches the DELIVERY_MODE destination (while TEST it MUST resolve to the owner's DM, never a channel or unresolved handle); (3) the message appears via slack_list_pending with the correct destination label and intended text (spot-check the title line and a couple of figures). If any fails: re-check the exact channel value from CONFIG, send once more, re-verify. If it still fails, do NOT report success: report the exact failure in the run output (and to the owner's DM if the connector permits).
In the completion output, list: queue id, resolved channel label, planned send time; NOW_ET and each bucket's Total / So-far / Remaining / Live / Show Rate; the Collective Show Rate and whether the HARD RULE 9 sub-40% DM override applied; whether the AVOMA ACCESS FALLBACK proxy was used; QA outcome (PASS and fix cycles run, or last-resort failure detail); any LESSON notes; anomalies (unattributed bookings excluded, new/ambiguous master-page source, zero-bookings flag, N/A show rate, off-hour timing, etc.); and whether DELIVERY VERIFICATION passed or exactly why it failed.
