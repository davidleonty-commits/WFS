...
GOOGLE ACCOUNT: Google Drive reads for this task run under the director's WORK Google account (david.leonty@thewfsgroup.com). The outgoing director additionally authorized his own personal Gmail here, because the Salesboard had been shared with it; that allowance was specific to him and is removed. If the connected Drive account is not the work account, stop and report to David's DM. In all cases, do not fall back to any other read method: never the gviz CSV export, never a browser.
