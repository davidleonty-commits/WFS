...
- One sender only: all sends go through `chat.postMessage` on the workspace bot token (see SLACK DELIVERY RULE). Never a personal user token, never a second sender.
...
- LENGTH CAP: every single Slack message body MUST be under 3000 characters. The connector renders each message as a Slack block whose text caps at 3000 characters; anything longer is silently chunked and the overflow escapes the thread. Count the characters of each body BEFORE calling `chat.postMessage`.
...
DELIVERY STRUCTURE (verified method): the summary is ONE standalone DM message; every per-call block is a THREADED COMMENT under it. Use channel + thread_ts.
  ...
  4. Send each per-call block via `chat.postMessage` with channel = DIRECTOR_SLACK_ID AND thread_ts = that ts, in numbered order, waiting for each call to return ok before sending the next so thread order is preserved.
  5. After ALL blocks have been sent, run ONE final verification pass with `conversations.replies` (channel = the channel id the sends returned, ts = the parent ts): confirm every block appears as a reply, in order.
...
STEP 3, SLACK (bot token; Message 1 standalone + threaded call reviews per the DELIVERY STRUCTURE):
  ...
  BEFORE SENDING, DUPLICATE GATE (hard abort):
    1. Read the DM with `conversations.history` (channel = DIRECTOR_SLACK_ID, limit 20).
  ...
  DELIVERY VERIFICATION: confirm Message 1's send returned ok with a ts, so exactly ONE top-level DM message exists; then, after all comments are sent, ONE final `conversations.replies` pass (channel = the returned DM channel id, ts = parent ts) confirming every call-reviews block appears as a reply under it. Report per-message whether it dispatched.
