...
created: 2026-07-20
connectors_required: Supabase, Callix, Slack, Google_Drive
---
...
CONNECTORS (load their tool schemas via ToolSearch first): Supabase (mcp__Supabase__execute_sql), Slack (`slack_send_message` on the connector above), and Callix (`mcp__Callix__list_calls`, or `GET $CALLIX_API_BASE/calls` with header `Authorization: Bearer $CALLIX_API_KEY` if the MCP is unavailable). If a required connector cannot be loaded, send the alert DM described below stating that the guard could not run and why, then stop.
...
4. For each SUSPECT weekday, corroborate with Callix: count that day's TTW consultation calls = recorded meetings whose title contains the exact phrase "TikTok Wiz Consultation" (exclude titles containing "S2C" or "sync"), not cancelled, recorded duration over 15:00, whose Mountain-Time date equals that day. Callix start times are not timezone tagged: treat as UTC and subtract 6 hours to get MT.
5. DECISION. If any SUSPECT weekday had Callix consultation calls but 0 (or fewer than 5) rows in lead_quality, the publisher missed or underran that run. Send ONE Slack DM to the director via `slack_send_message` (channel = DIRECTOR_SLACK_ID, sent immediately; on failure retry once, since there is no second sender). The DM lists each affected day with: Callix consultation-call count vs rows written in lead_quality, and one line noting the Daily Call Report Publisher likely failed that run and the day needs a backfill. Keep it short and factual. No em dashes.
6. If NO weekday in the window is suspect (every recent weekday has rows roughly matching its Callix call count), send NOTHING and end silently.
