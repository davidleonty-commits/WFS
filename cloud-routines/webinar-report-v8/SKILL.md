...
DATA SOURCES
=====================================================
ONCEHUB (REST API, read-only): `GET https://api.oncehub.com/v2/<resource>` with header `API-Key: $ONCEHUB_API_KEY`. Endpoints: `/v2/bookings`, `/v2/bookings/{id}`, `/v2/master_pages`, `/v2/master_pages/{id}`, `/v2/booking_pages/{id}`.
...
- `/v2/master_pages` MUST be paginated: the account has 216 master pages over 3 pages, and "S2C | Demo | Closer | 45min" appears only on page 3. A single unpaginated call silently misses pages.
...
STEP 9: Deliver
=====================================================
After QA passes only. Destination = TEST_TARGET in TEST (never LIVE_TARGET), LIVE_TARGET in LIVE. `chat.postMessage` with text = the finished report exactly as built and channel = that destination, sent immediately. Mentions, if any, are inline <@MEMBERID> tokens in the text and nothing else. On a transient 503 retry once. Capture the returned ts and resolved channel as the delivery proof. Send exactly once.
