...
DATA SOURCES
=====================================================
ONCEHUB (Lovable WFS connector, read-only). Tools: oncehub_list_master_pages, oncehub_list_bookings, oncehub_get_booking, oncehub_get_master_page, oncehub_booking_counts, oncehub_api, and (connector v1.2.0+) oncehub_find_bookings_by_attendee.
CONNECTOR NOTES (apply HARD RULE 12):
- ENHANCED list_bookings (v1.2.0+): starting_time_from / starting_time_to map onto the upstream starting_time.gt / .lt; exclude_trash replaces in_trash; fields projects a subset (use ['id','status','in_trash','master_page','starting_time','form_submission']); expand_master_page:true adds master_page_label inline; fetch_all:true sweeps exhaustively; responses carry has_more / next_after / returned.
- BASELINE FALLBACK (verified working 2026-07-27): starting_time_from / starting_time_to / in_trash are REJECTED upstream with HTTP 400 "Received unknown parameter". Use oncehub_api on /v2/bookings with the raw operators starting_time.gt and starting_time.lt, limit 100, paginating with after = the LAST BKNG- id until a page returns fewer than limit. On the old build there is NO next cursor even when more remain, so a page returning exactly limit ALWAYS means fetch again.
- ALWAYS filter trashed client-side on each booking's own in_trash field. Upstream trash behavior is inconsistent and there is no reliable server-side filter on the old build.
- oncehub_list_master_pages MUST be paginated: the account has 216 master pages over 3 pages, and "S2C | Demo | Closer | 45min" appears only on page 3. A single unpaginated call silently misses pages.
- oncehub_booking_counts is a CROSS-CHECK ONLY, never a published number, and only when it reports truncated:false (the old build silently capped at one page, reporting total_all 100 with truncated false when the truth was 138).
- oncehub_find_bookings_by_attendee (v1.2.0+) matches email and/or partial name over a date range. Use it for the source-page lookups in STEPS 3 and 4. DO NOT use it for double-booking detection: it dedupes on lowercased email, and all three verified double bookings used DIFFERENT addresses (carmen_mercer2000 vs carmen_mercer2025; jaelyn0717@yahoo.con vs .com; jdippi1971 vs jmirand71).
...
STEP 0: Start
=====================================================
Note today's date and day (America/Denver). Confirm the connectors are reachable and RECORD which OnceHub capabilities exist (does oncehub_find_bookings_by_attendee exist; does list_bookings accept fetch_all / fields / expand_master_page / exclude_trash), since that decides which path the run takes. If a required connector is unavailable, enter STEP 7B. Supabase unreachable is NOT a blocker: DQ = N/A and continue.

=====================================================
STEP 1: Target webinar and cohort bookings (OnceHub)
=====================================================
1. oncehub_list_master_pages, PAGINATED to the end. Webinar CLOSER pages are labeled "Webinar MM DD YY | Paid | Closer | 45min". When selecting the TARGET ignore Setter pages, "Webinar S2C", and "Automated Webinar". Parse each dated closer webinar's date and sort. 2. TARGET = the most recent webinar at least 4 days before today (MT). 3. NEXT WEBINAR = the nearest dated closer webinar strictly later than TARGET. 4. Webinar Date = the TARGET page's parsed date (report as MM/DD). 5. oncehub_list_bookings with master_page = TARGET page id. PREFERRED: fetch_all:true, exclude_trash:true, fields projection. FALLBACK: limit 100 paginating with after = the LAST BKNG- id until a page returns fewer than limit; do NOT pass in_trash or starting_time_from/to on that path. Filter trashed client-side either way. 6. COUNTING: count "object": "booking" records and cross-check distinct BKNG- ids; the two MUST agree. NEVER grep "status": lines, since the envelope's "status": 200 contaminates the count (it once gave 63 vs the true 64). 7. ALL COHORT BOOKINGS = every non-trashed booking on the page, any status, any starting_time. Keep form_submission.name and .email for each. 8. IN-WINDOW COHORT BOOKINGS = the subset whose starting_time (converted to MT) falls inside the STEP 2 window. COHORT Closer Calls Total = the count of these. COHORT Canceled = those with status "canceled". Bookings OUTSIDE the window are excluded from all cohort figures; list them in STEP 10 with date and status, split into "before window" and "still upcoming", since upcoming ones mean the webinar's cycle is unfinished.
...
=====================================================
Sweep every booking whose starting_time falls in the window. PREFERRED: list_bookings with starting_time_from = <window start>T06:00:00Z, starting_time_to = <window end + 1 day>T05:59:59Z, fetch_all:true, exclude_trash:true, expand_master_page:true, fields projection. FALLBACK: oncehub_api /v2/bookings with starting_time.gt and starting_time.lt, limit 100, paginating with after, then resolve labels via the PAGINATED master-page list. Filter trashed client-side.
