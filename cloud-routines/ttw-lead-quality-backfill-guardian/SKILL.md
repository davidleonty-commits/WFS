...
HARD RULES
- Read-only EXCEPT: (a) upserting missing rows into public.lead_quality, (b) inserting one row per checked day into public.report_run_log. Never delete or overwrite existing lead_quality rows beyond the defined upsert. Never post to any client channel. Never re-post the daily client report. The ONLY Slack send you may make is a direct message to the owner (Slack user id U092C85GA4D), and only on a shortfall.
...
- AVOMA FALLBACK if the native Avoma MCP is down (502 or not connected): use the WFS connector avoma_api passthrough. Meetings: avoma_api path="/v1/meetings/" query={from_date,to_date,page,page_size}. Single transcript: avoma_api path="/v1/transcriptions/{transcription_uuid}/" (the response has data.transcript[] with speaker_id and text and data.speakers[] with is_rep).
...
For each missing meeting_uuid, pull get_meeting_transcript(uuid) (or the avoma_api transcription fallback). Avoma mislabels speaker names, so infer rep vs prospect from context (the rep pitches the TTW or Inner Circle offer; the prospect is the buyer). Score with the SAME rubric the publisher uses:
...
If EVERY target day is OK (missing_before = 0 for all), send NOTHING and end silently. Otherwise, DM the owner (Slack user id U092C85GA4D) via the WFS connector slack_schedule_message (jitter_minutes 0), plain text per the formatting rules, structured as:
