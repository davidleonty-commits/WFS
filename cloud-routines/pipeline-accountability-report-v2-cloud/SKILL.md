...
REFERENCE MAPPING (see MAPPING VERIFICATION POLICY below) = each row as "Full Name | Pipedrive Owner ID" from the roster's Pipedrive Owner ID column. That integer owner id is the ONLY rep key this report joins on, for deals and activities alike; the roster's old WFS connector id column is dead and must not be used.
