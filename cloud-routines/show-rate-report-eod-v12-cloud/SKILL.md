...
created: 2026-07-16
connectors_attached: Avoma_MCP, Canva, Claude_Code_Remote, ClickUp, Excalidraw, Google_Calendar, Google_Drive, HyperFrames_by_HeyGen, Lovable, Lovable_WFS_Slack, Pipedrive_MCP, Slack, Supabase
---
...
TEST_TARGET: U092C85GA4D
  (Owner's DM, Cayden Johnson. ALWAYS the destination for QA FAILURE and RECONCILIATION reports in either mode. Addressed by Slack user ID because "@cayden" and "cayden.johnson@thewfsgroup.com" do NOT resolve through the WFS connector; U092C85GA4D is confirmed working, resolving to "@Cayden Johnson". Pass this ID verbatim as the channel.)
...
=====================================================
All Slack SENDING goes through the Lovable WFS Slack connector's slack_schedule_message tool. The native Slack connector must NEVER be used to send anything; it is permitted for READ ONLY, and only for the #payments cross-check in STEP 2D.
Delivery step: destination = TEST_TARGET if TEST, LIVE_TARGET if LIVE (never LIVE_TARGET while in TEST). Call slack_schedule_message with text = the finished message, channel = that destination, jitter_minutes = JITTER_MINUTES, agent = "claude-cowork", and NO send_at_mt. Capture the returned queue id, resolved channel label, and planned send time. Send the report exactly once.
ERROR HANDLING: 401/Unauthorized -> token missing/rotated; report that WFS_MCP_TOKEN needs refreshing in Settings -> AI Agent Access. Destination does not resolve -> in TEST, the confirmed-working target is the Slack user ID in TEST_TARGET; in LIVE, verify the exact channel name from CONFIG. "not in the approved allowlist" (LIVE only) -> report; do NOT fall back to the owner DM for the report body. "Slack is not connected" -> report. If slack_schedule_message is unavailable, report that the Lovable WFS MCP connector is not connected; never improvise another delivery path.

=====================================================
HARD RULES (never violate)
=====================================================
1. SENDING is WFS-connector-only. Per run this task may send AT MOST: (a) exactly ONE report to the DELIVERY_MODE target, (b) at most ONE reconciliation DM to the owner (STEP 2E), and (c) at most ONE QA FAILURE DM to the owner (last resort). Nothing else. The slack_list_pending verification read and the STEP 2D #payments read are reads, not sends.
...
8. SELF-HEAL GUARDRAILS: the fix loop may change METHODS but never POLICIES: never alter the CONFIG block, never add write targets, never use the native Slack connector to send, never substitute a different source of truth (OnceHub for scheduled, Avoma for live, PIPEDRIVE for closed deals and collected dollars), never promote the Salesboard or Slack to a published figure, never weaken a HARD RULE.
...
METHOD: oncehub_booking_counts cannot filter by name, so subtract test records identified in the STEP 1B-4 per-booking lists from the booking_counts figures. Record counts, ids and names as run-output flags.
...
1B-1 RESCHEDULED (R): oncehub_booking_counts, starting_time, today ET, status=["rescheduled"]. Sum ONLY closer-scope rows, minus test records.
...
1B-4 DOUBLE BOOKINGS (D): a today closer call counts as a double booking when the SAME EXACT customer name is booked on another closer call at a DIFFERENT time within the current Monday-Sunday ET week, with at least one booking falling today.
  ...
  - METHOD: oncehub_list_bookings accepts starting_time_from and starting_time_to together with fetch_all=true. Call ONCE PER closer master page with master_page=<id>, fetch_all=true, fields=['id','status','in_trash','master_page','starting_time','form_submission','rescheduled_booking_id','cancel_reschedule_information']. The tool maps bounds to strict gt/lt, so widen the window by about an hour each side and filter CLIENT-SIDE to the exact week. Confirm per page that total == returned, has_more = false, truncated = false.
...
2B-4 REP NAME RESOLUTION. The deal carries owner_id as a BARE INTEGER at top level. Do NOT use creator_user_id: it is almost always 23739826, the automation account. There is NO Pipedrive user endpoint on this connector (getUser / getUsers do not exist; verified), so names must be resolved by join.
  Build the map at runtime each run so a new rep resolves automatically:
    a) mcp__Lovable_WFS_Slack__pipeline_smart_view(view="deals_won_last_7d", limit=100) returns items with title, value, won_time and rep_id.
    b) mcp__Lovable_WFS_Slack__reps_scoreboard(window_days=14) maps rep_id to name.
    c) Join your Pipedrive deals to those items on (title, won_time). won_time matches to the second across both systems, so the join is exact.
    d) Seed and backstop with PIPEDRIVE_OWNER_MAP_SEED.
  Map by rep_id, NEVER by name string: reps_scoreboard returns duplicate names across different rep_ids (two "Cayden Johnson", two "Tom Judson", two "Peter Borreggine", two "Edward Woltz", two "Paul Rasoumoff").
  LIMITATION: this join only resolves reps who have a WON deal, so a rep whose deals sit status="open" in stage 16 may be unresolvable until they win one.
...
=====================================================
This step produces NO published figure. Read-only, and the ONLY permitted use of the native Slack connector in this task. Its job is not just a rough count comparison: it is a NAMED-CUSTOMER completeness check whose purpose is to catch a real sale that never made it into Pipedrive, since Pipedrive being PRIMARY only holds up if every closed deal actually lands there.
Read PAYMENTS_CHANNEL_ID using mcp__Slack__slack_read_channel (load via ToolSearch with "select:mcp__Slack__slack_read_channel"), passing channel_id = PAYMENTS_CHANNEL_ID and unix-epoch oldest/latest bounds, over a window covering today 00:00 MT through PULL_TIME, PLUS last night 22:00 MT through today 08:00 MT. Use ONLY this read tool from that connector; never mcp__Slack__slack_send_message, slack_schedule_message or any other Slack write tool. If the channel has more history than one page returns, paginate with the cursor until exhausted or until you are clearly past the window on both ends — do not settle for a single truncated page when checking completeness matters.
...
If the native Slack read tool is unavailable, record the Slack side as UNAVAILABLE and continue; this never blocks the report.
...
6. RULES: no writes to any OnceHub, Avoma, Pipedrive or Salesboard object; the native Slack connector was used for READING #payments only and never to send; delivery target matches DELIVERY_MODE; at most one report send, one reconciliation DM and one QA failure DM will occur; NO emoji anywhere. Any violation fails.
...
4. LAST RESORT: if the failure persists after 2 cycles, or the cause is a genuine external outage no method can fix, send ONE short QA FAILURE DM to TEST_TARGET naming the failed checks, the values involved, and every fix attempted. When a new closer master page is the cause, include its id, label, name and booking url, and state what the affected figures would be if it were added to the correct bucket. When a new Pipedrive owner_id is the cause, include the id and the deals attributed to it. If a fix cycle corrected a method or uncovered an environment quirk, include a LESSON note. Then stop without delivering. Never post a known-bad report to the client channel.
   EXCEPTIONS that do NOT block delivery: a Pipedrive outage (publish without the Sales section per 2B-10), a Google Drive outage (Salesboard side recorded UNAVAILABLE), or a native Slack read failure (Slack side recorded UNAVAILABLE). Those are reported in the reconciliation DM instead.
...
=====================================================
Deliver per the DELIVERY block: ONE slack_schedule_message call, text = the STEP 5 body only, channel = the DELIVERY_MODE destination, jitter_minutes = JITTER_MINUTES, agent = "claude-cowork", no send_at_mt.
DELIVERY VERIFICATION (run is NOT complete until this passes): (1) ok = true with a non-empty queue id and a resolved channel label; (2) the resolved label matches the DELIVERY_MODE destination (LIVE MUST resolve to #wfs-ttw-sales-mgmt-client; TEST MUST resolve to the owner's DM); (3) the message appears via slack_list_pending with the correct destination and intended text (spot-check the title line and a couple of figures). NOTE: a message planned for essentially now may already have dispatched and not appear in slack_list_pending; an empty pending list together with ok=true and a correctly resolved channel label is a SUCCESS, not a failure. Do not re-send on an empty pending list alone. If (1) or (2) fails: re-check the exact channel value from CONFIG, send once more, re-verify. If it still fails, do NOT report success.
Then send the STEP 2E reconciliation DM if and only if the divergence test triggered.
Then report in your run output: PULL_TIME (MT) and the lo_utc/hi_utc window used; queue id, resolved channel label, planned send time; whether the reconciliation DM was sent and its contents; QA result (PASS and how many self-heal cycles ran, or the last-resort failure detail plus any LESSON notes); and all data flags.
