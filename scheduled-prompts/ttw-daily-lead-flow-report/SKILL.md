...
description: Weekday TTW Daily Lead Flow Report: pull OnceHub via its REST API (no browser scraping), compute the report, QA-gate, and deliver with the Slack workspace bot token (LIVE: #wfs-ttw-sales-mgmt-client).
---
...
2. DELIVERY: all Slack delivery goes through `chat.postMessage` on the workspace bot token. Never a personal user token, never a second sender. While DELIVERY_MODE is TEST the destination is ALWAYS the DM (TEST_TARGET); never LIVE_TARGET in TEST. Send exactly once.
...
5. Never enter credentials anywhere, and never put a key in the message body or the archive doc. The OnceHub key and the Slack token are read from the environment (`$ONCEHUB_API_KEY`, `$SLACK_BOT_TOKEN`) and never printed.
...
STEP 0: STARTUP
=====================================================
Confirm OnceHub access (a `GET /v2/master_pages` returning rows) and Slack send access (an `auth.test` call returning ok on the bot token). If the OnceHub key is missing or every OnceHub call errors, STOP and report — do NOT fall back to the browser for data.
