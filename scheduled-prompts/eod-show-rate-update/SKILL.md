...
description: Weekday 7pm MT end-of-day Webinar + S2C show rate report, connector-based (no browser): scheduled + booking-health counts from the WFS OnceHub MCP, live calls from Avoma MCP, full-day scheduled counts (NO halving). Also reports today's closer-call reschedules, cancellations, double bookings, and the scheduled-vs-category %. QA gate with self-healing fix loop, delivers via Lovable WFS slack_schedule_message. LIVE as of 07/09/26 to #wfs-ttw-sales-mgmt-client (client-facing).
---
...
TEST_TARGET: U092C85GA4D
  (Owner's DM (Cayden Johnson). The only destination allowed while DELIVERY_MODE is TEST, and ALWAYS the destination for QA FAILURE reports regardless of mode. Addressed by Slack user ID because "@cayden" and the WFS email "cayden.johnson@thewfsgroup.com" do NOT resolve through the WFS connector; the ID U092C85GA4D is confirmed working, resolving to "@Cayden Johnson". Pass this ID verbatim as the channel.)
...
=====================================================
DELIVERY (through the Lovable WFS Slack connector only)
=====================================================
Never use the native Slack connector. All Slack delivery goes through the Lovable WFS Slack connector's slack_schedule_message tool.
Delivery step: determine the destination from DELIVERY_MODE (TEST_TARGET if TEST, LIVE_TARGET if LIVE; never LIVE_TARGET while in TEST). Call slack_schedule_message with text = the finished message, channel = that destination, jitter_minutes = JITTER_MINUTES, agent = "claude-cowork", and NO send_at_mt (send as soon as possible). Capture the returned queue id, resolved channel label, and planned send time. Send exactly once.
ERROR HANDLING: 401/Unauthorized -> token missing/rotated; report that WFS_MCP_TOKEN needs refreshing in Settings -> AI Agent Access. Destination does not resolve -> in TEST, the confirmed-working target is the Slack user ID in TEST_TARGET; in LIVE, verify the exact channel name from CONFIG is being passed. "not in the approved allowlist" (LIVE only) -> report; do NOT fall back to the owner DM for the report body, only for a QA FAILURE notice. "Slack is not connected" -> report. If the slack_schedule_message tool is not available in this session at all, report that the Lovable WFS MCP connector is not connected; do not improvise another delivery path. Connector-down conditions are last-resort conditions for the SELF-HEAL loop, not license to use another sender.

=====================================================
HARD RULES (never violate)
=====================================================
1. Delivery is WFS-connector-only, exactly once. The only Slack actions this task takes are the single slack_schedule_message send to the DELIVERY_MODE target (plus, in the last-resort case only, one QA FAILURE report to the owner's DM) and the slack_list_pending verification read.
...
OBJECTIVE
=====================================================
Pull an END-OF-DAY show rate report covering Webinar and S2C for TODAY only, with scheduled counts from OnceHub (WFS OnceHub MCP) and live counts from Avoma. By end of day essentially all scheduled calls have already occurred, so use the FULL day's scheduled (non-canceled) count as the show-rate denominator: NO halving. In addition to the two show-rate sections, the report includes a closer-call BOOKING HEALTH section for today: the number of rescheduled calls, canceled calls, and double bookings, plus the share of today's closer calls that were scheduled cleanly vs. fell into one of those three categories (STEP 1B). Deliver via one slack_schedule_message call through the WFS connector to the DELIVERY_MODE destination, only after the QA gate passes.
Sources:
- OnceHub via the connected WFS OnceHub MCP. oncehub_booking_counts (grouped by master page) is the source of truth for scheduled Webinar and S2C closer calls and for the today reschedule/cancel counts; oncehub_list_bookings (booking-level, includes form_submission.name) is the source for the double-booking name match; oncehub_get_master_page resolves any unlabeled master_page_id. These are the same numbers the TTW Daily Lead Flow Report and the midday show rate task are built from.
...
=====================================================
Pull today's active bookings grouped by master page with ONE oncehub_booking_counts call:
- date_field = starting_time
- date_from = today 00:00:00 ET, date_to = today 23:59:59 ET (pass the correct ET offset, -04:00 EDT or -05:00 EST)
...
oncehub_booking_counts excludes in_trash by default and is known-good for daily starting_time slices (verified against a full pagination on 2026-07-02). Read counts per master page from rows. Some rows return an unresolved master_page_id as the label; resolve any such row with oncehub_get_master_page to read its name and label. Unattributed bookings (total_all minus total_attributed) are EXCLUDED from every count; note the number as a run-output flag.
...
1B-1 RESCHEDULED TODAY (R): oncehub_booking_counts, date_field=starting_time, today ET window, status=["rescheduled"]. Sum ONLY the closer-scope rows (any "Webinar ... | Paid | Closer" page + the two S2C pages; drop Setter and organic/automated rows). R = that sum.
1B-2 CANCELED TODAY (C): oncehub_booking_counts, date_field=starting_time, today ET window, status=["canceled"]. Sum ONLY the closer-scope rows, including any Webinar Closer page absent from STEP 1. C = that sum.
...
5. LAST RESORT ONLY: if after 5 cycles the failure persists AND the cause is a genuine external outage or source problem no method can fix (a required connector hard-down all run, OnceHub returning no data that will not reconcile, a total that will not reconcile), send ONE short QA FAILURE DM to the owner (TEST_TARGET U092C85GA4D) naming the failed checks, the values involved, and every fix attempted. QA FAILURE reports ALWAYS go to the owner's DM, even when DELIVERY_MODE is LIVE. Then stop without delivering. Never post a known-bad report to the client channel.

=====================================================
STEP 7: Deliver + run output (only after QA passes)
=====================================================
Deliver per the DELIVERY block: make ONE slack_schedule_message call with text = the STEP 5 Slack body (clean body only), channel = the DELIVERY_MODE destination (LIVE_TARGET while LIVE), jitter_minutes = JITTER_MINUTES, agent = "claude-cowork", and no send_at_mt. Send exactly once.
DELIVERY VERIFICATION (the run is NOT complete until this passes): (1) the call returned ok = true with a non-empty queue id and a resolved channel label; (2) the resolved label matches the DELIVERY_MODE destination (while LIVE it MUST resolve to #wfs-ttw-sales-mgmt-client; while TEST it MUST resolve to the owner's DM, never a channel or unresolved handle); (3) the message appears via the WFS connector's slack_list_pending with the correct destination label and intended text (spot-check the title line and a couple of figures). NOTE (L5): a message whose planned send time is essentially now may already have dispatched and therefore not appear in slack_list_pending; an empty pending list together with an ok=true send and a correctly resolved channel label is a successful send, not a failure. If (1) or (2) fails: re-check the exact channel value from CONFIG, send once more, and re-verify. If it still fails, do NOT report success: report the exact failure in the run output (and to the owner's DM if the connector permits) so the operator knows the report was not delivered.
...
L4 (07/07/26): SOURCE MIGRATED off the Leadflow Google Doc. The team stopped updating the doc, so the doc showed no section for the current day and the old date-mismatch stop condition fired every run. Per owner directive this task now reads scheduled counts from the WFS OnceHub MCP (oncehub_booking_counts, date_field=starting_time, ET day window, status scheduled/rescheduled/completed/no-show to exclude canceled), identical to the midday-show-rate-update and ttw-daily-lead-flow-report methods, but with NO halving (full-day count is the EOD denominator). Classification: Webinar Scheduled = sum of "Webinar MM DD YY | Paid | Closer | 45min" pages (classify by label, ids rotate per webinar); S2C Scheduled = BOTH S2C demo pages combined, BP-B0F8QC4ELN + BP-CQW4ZPAJKG; IGNORE Setter pages and organic/automated closer sources BP-39EQN7T28V (YouTube), BP-NR607FUT3C (Newsletter), BP-8KJ6WF27AV (Automated Webinar Closer). Some rows return the raw master_page_id as label; resolve with oncehub_get_master_page or the KNOWN MASTER-PAGE ID REFERENCE. The old date-mismatch stop condition is retired.
L5 (07/07/26): a slack_schedule_message with no send_at_mt and planned send time at essentially the current moment can dispatch immediately and then NOT appear in slack_list_pending; an empty pending list alongside an ok=true send that resolved to the correct channel label is a successful send, not a delivery failure. Do not re-send on an empty pending list alone.
L6 (07/07/26): OWNER-ADDED closer BOOKING HEALTH to the report (STEP 1B): today's Rescheduled (R), Canceled (C), and Double Bookings (D), plus Scheduled Cleanly % vs In a Category %. Scope is closer-only (Webinar Closer + the two S2C pages), owner-confirmed. Reschedule/cancel counts come from oncehub_booking_counts with status=["rescheduled"] and status=["canceled"] on today's ET window, summing only closer rows. Double bookings use oncehub_list_bookings for booking-level form_submission.name: the tool does NOT accept starting_time_from/starting_time_to (the OnceHub API rejects those params, confirmed 07/07/26), so pull per closer master_page and filter starting_time CLIENT-SIDE to the Monday-Sunday ET week, then flag a today closer call when the same EXACT trimmed name (case-insensitive) appears on another distinct closer booking at a different time in the week, EXCLUDING reschedule-linked same-appointment pairs. Base for the % is A + C (active plus canceled); the issue count is deduped by booking id so it is not simply R + C + D.
