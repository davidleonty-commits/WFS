...
description: Weekday 7pm MT end-of-day Webinar + S2C show rate report, connector-based (no browser): scheduled + booking-health counts from the OnceHub API, live calls from Avoma MCP, full-day scheduled counts (NO halving). Also reports today's closer-call reschedules, cancellations, double bookings, and the scheduled-vs-category %. QA gate with self-healing fix loop, delivers via one `slack_send_message` call on the Slack connector. The client channel is OFF LIMITS under the new director; delivery is the director's DM.
---
